...
import net.divinerpg.uvite.gen.TeleporterUvite;
...
public class BlockUvitePortal extends BreakableBase
{

    public BlockUvitePortal(int par1)
    {
        super(par1, "Uvite", Material.portal, false);
    ...
    public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
    {
        byte b0 = 0;
        byte b1 = 0;
        if (par1World.getBlockId(par2 - 1, par3, par4) == TwilightBlockHelper.azuriteBlock.blockID || par1World.getBlockId(par2 + 1, par3, par4) == TwilightBlockHelper.azuriteBlock.blockID)
        {
            b0 = 1;
        }
        if (par1World.getBlockId(par2, par3, par4 - 1) == TwilightBlockHelper.azuriteBlock.blockID || par1World.getBlockId(par2, par3, par4 + 1) == TwilightBlockHelper.azuriteBlock.blockID)
        {
            b1 = 1;
        }
        if (b0 == b1)
        ...
        else
        {
            ...
            for (l = -1; l <= 2; ++l)
            {
                for (i1 = -1; i1 <= 3; ++i1)
                {
                    ...
                    if (l != -1 && l != 2 || i1 != -1 && i1 != 3)
                    {
                        ...
                        if (flag)
                        {
                            if (j1 != TwilightBlockHelper.azuriteBlock.blockID)
                                return false;
                        }
            ...
            for (l = 0; l < 2; ++l)
            {
                for (i1 = 0; i1 < 3; ++i1)
                {
                    par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l, TwilightBlockHelper.UvitePortal.blockID, 0, 2);
                }
            }
            return true;
        }
    ...
    public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5)
    {
        ...
        if (par1World.getBlockId(par2, i1 - 1, par4) != TwilightBlockHelper.azuriteBlock.blockID)
        {
        ...
        else
        {
            ...
            if (j1 == 3 && par1World.getBlockId(par2, i1 + j1, par4) == TwilightBlockHelper.azuriteBlock.blockID)
            {
                ...
                {
                    if ((par1World.getBlockId(par2 + b0, par3, par4 + b1) != TwilightBlockHelper.azuriteBlock.blockID || par1World.getBlockId(par2 - b0, par3, par4 - b1) != this.blockID) && (par1World.getBlockId(par2 - b0, par3, par4 - b1) != TwilightBlockHelper.azuriteBlock.blockID || par1World.getBlockId(par2 + b0, par3, par4 + b1) != this.blockID))
                    {
                        par1World.setBlockToAir(par2, par3, par4);
                    }
                }
    ...
    public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity)
    {
        if ((par5Entity.ridingEntity == null) && (par5Entity.riddenByEntity == null) && ((par5Entity instanceof EntityPlayerMP)))
        {
            ...
            else if (thePlayer.dimension != DimensionConfigHelper.UviteID)
            {
                thePlayer.timeUntilPortal = 10;
                thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, DimensionConfigHelper.UviteID, new TeleporterUvite(thePlayer.mcServer.worldServerForDimension(DimensionConfigHelper.UviteID)));
            }
            else {
                thePlayer.timeUntilPortal = 10;
                thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new TeleporterUvite(thePlayer.mcServer.worldServerForDimension(0)));
            }
        }
    }
    @Override
    ...
    {
        return TwilightBlockHelper.UvitePortal.blockID;
    }
}