...
import net.divinerpg.augite.gen.TeleporterAugite;
...
import net.divinerpg.render.portalFX.EntityAugitePortalFX;
...
public class BlockAugitePortal extends BreakableBase
{

    public BlockAugitePortal(int par1)
    {
        super(par1, "Augite", Material.portal, false);
    ...
    public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
    {
        byte b0 = 0;
        byte b1 = 0;
        if (par1World.getBlockId(par2 - 1, par3, par4) == TwilightBlockHelper.mythrilBlock.blockID || par1World.getBlockId(par2 + 1, par3, par4) == TwilightBlockHelper.mythrilBlock.blockID)
        {
            b0 = 1;
        }
        if (par1World.getBlockId(par2, par3, par4 - 1) == TwilightBlockHelper.mythrilBlock.blockID || par1World.getBlockId(par2, par3, par4 + 1) == TwilightBlockHelper.mythrilBlock.blockID)
        {
            b1 = 1;
        }
        if (b0 == b1)
        ...
        else
        {
            ...
            for (l = -1; l <= 2; ++l)
            {
                for (i1 = -1; i1 <= 3; ++i1)
                {
                    ...
                    if (l != -1 && l != 2 || i1 != -1 && i1 != 3)
                    {
                        ...
                        if (flag)
                        {
                            if (j1 != TwilightBlockHelper.mythrilBlock.blockID)
                                return false;
                        }
            ...
            for (l = 0; l < 2; ++l)
            {
                for (i1 = 0; i1 < 3; ++i1)
                {
                    par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l, TwilightBlockHelper.AugitePortal.blockID, 0, 2);
                }
            }
            return true;
        }
    ...
    public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5)
    {
        ...
        if (par1World.getBlockId(par2, i1 - 1, par4) != TwilightBlockHelper.mythrilBlock.blockID)
        {
        ...
        else
        {
            ...
            if (j1 == 3 && par1World.getBlockId(par2, i1 + j1, par4) == TwilightBlockHelper.mythrilBlock.blockID)
            {
                ...
                {
                    if ((par1World.getBlockId(par2 + b0, par3, par4 + b1) != TwilightBlockHelper.mythrilBlock.blockID || par1World.getBlockId(par2 - b0, par3, par4 - b1) != this.blockID) && (par1World.getBlockId(par2 - b0, par3, par4 - b1) != TwilightBlockHelper.mythrilBlock.blockID || par1World.getBlockId(par2 + b0, par3, par4 + b1) != this.blockID))
                    {
                        par1World.setBlockToAir(par2, par3, par4);
                    }
                }
    ...
    public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity)
    {
        if ((par5Entity.ridingEntity == null) && (par5Entity.riddenByEntity == null) && ((par5Entity instanceof EntityPlayerMP)))
        {
            ...
            else if (thePlayer.dimension != DimensionConfigHelper.AugiteID)
            {
                thePlayer.timeUntilPortal = 10;
                thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, DimensionConfigHelper.AugiteID, new TeleporterAugite(thePlayer.mcServer.worldServerForDimension(DimensionConfigHelper.AugiteID)));
            }
            else {
                thePlayer.timeUntilPortal = 10;
                thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new TeleporterAugite(thePlayer.mcServer.worldServerForDimension(0)));
            }
        }
    }
    @Override
    ...
    {
        return TwilightBlockHelper.AugitePortal.blockID;
    }
}