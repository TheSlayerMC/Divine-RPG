...
public class BlockAntiNetherPortal extends BreakableBase
{

    public BlockAntiNetherPortal(int par1)
    {
    ...
    public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
    {
        ...
        else
        {
            ...
            for (l = 0; l < 2; ++l)
            {
                for (i1 = 0; i1 < 3; ++i1)
                {
                    par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l, OverworldBlockHelper.AntiNetherPortal.blockID, 0, 2);
                }
            }
            return true;
        }
    ...
    public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5)
    {
        ...
        if (par1World.getBlockId(par2, i1 - 1, par4) != Block.netherrack.blockID)
        {
        ...
        else
        {
            ...
            if (j1 == 3 && par1World.getBlockId(par2, i1 + j1, par4) == Block.netherrack.blockID)
            {
                ...
                {
                    if ((par1World.getBlockId(par2 + b0, par3, par4 + b1) != Block.netherrack.blockID || par1World.getBlockId(par2 - b0, par3, par4 - b1) != this.blockID) && (par1World.getBlockId(par2 - b0, par3, par4 - b1) != Block.netherrack.blockID || par1World.getBlockId(par2 + b0, par3, par4 + b1) != this.blockID))
                    {
                        par1World.setBlockToAir(par2, par3, par4);
                    }
                }
    ...
    public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity)
    {
        if ((par5Entity.ridingEntity == null) && (par5Entity.riddenByEntity == null) && ((par5Entity instanceof EntityPlayerMP)))
        {
            ...
            else if (thePlayer.dimension != DimensionConfigHelper.antiNetherID)
            {
                thePlayer.timeUntilPortal = 10;
                thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, DimensionConfigHelper.antiNetherID, new TeleporterAntiNether(thePlayer.mcServer.worldServerForDimension(DimensionConfigHelper.antiNetherID)));
            }
            else {
                thePlayer.timeUntilPortal = 10;
                thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new TeleporterAntiNether(thePlayer.mcServer.worldServerForDimension(0)));
            }
        }
    }
    @Override
    ...
    {
        return OverworldBlockHelper.AntiNetherPortal.blockID;
    }
}